...
public class Receiver {
	...
	public static void main(String[] args) {
		if(args.length != 0 && args[0].equals("--help")){
			System.out.println("Usage: java -jar CubaseSimulator.jar <Listen-Port> [<OSC-Listen-Path>] [<OSC-Listen-Path>] ...");
		} else if(args.length != 0) {
			Receiver rec = new Receiver(Integer.parseInt(args[0]), args);
		}
	}
	
	public Receiver (int port, String[] args) {
					...
					listener = new OSCListener() {						
						public void acceptMessage(java.util.Date time, OSCMessage message) {
							...
							System.out.println(new Timestamp(date.getTime())+": Got a Message: " +
									"Track: "+path+" "+ 
									"Gain: "+(float)message.getArguments()[0]);
						}
					};					
